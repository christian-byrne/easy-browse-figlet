"""Preview figlet fonts for different font sizes and messages. 

Ideally, pipe to more or less.
...
fonts = {
    ...
    "s": ["doom", "bulbhead", "starwars", "o8", "ntgreek", "maxfour", "letters", "elevator"],
    "xs": ["bubble", "contessa", "cybersmall", "digital", ],
    "favorites": ["clb8x10", "cyberlarge", "broadway", "5lineoblique", "fuzzy", "thin", "charact2", "rounded", "poison", "fraktur", "starwars", "big", "smslant", "graffiti", "sblood", "xsbook", "epic", "contrast", "doom", "roman", "univers", "doh", "rev"]
}


def get_args():
    arg_parser = argparse.ArgumentParser(
        description="Preview different figlet fonts for specified text and desired font size range.")
    arg_parser.add_argument("--size", "-s", choices=[
                            "xs", "s", "m", "l", "xl", "xxl", "all"], nargs="*", default="all", metavar="XX", )
    arg_parser.add_argument("--fonts", "-f", nargs="*",
                            default="all", metavar="FONT", )
    arg_parser.add_argument(
        "--favorites", action="store_true", help="Only print favorited fonts")
    arg_parser.add_argument("message", default="Hello World")
    arg_parser.add_argument("--log-errors", action="store_true",
                            help="Display log of font names that couldn't be found")

    return vars(arg_parser.parse_args())


def figify(message, font):
    verbose = get_args()["log_errors"]
    try:
        formatter = Figlet(font=font)
        def figg(msg): print(formatter.renderText(msg, ))

        print(term_gui.make_header(80, font, auto_capitalize=False))
        figg(message)
    # except KeyError or FontNotFound:
    except:
        if verbose:
            print(
                term_gui.make_header(
                    60, f"{font} FontNotFound or KeyError", centered=True, boxed=True, character="*")
            )
        return


def by_category(category, msg):
    def sep(n=1): print(SPACER*n)
    sep()
    print(
        term_gui.make_header(80, f"Previewing {category} figlet fonts. . .", centered=True, tiers=2)
    )

    selected = fonts[category]

    for ffont in selected:
        figify(msg, ffont)
        sep()
...
def main():
    ...
    if options["fonts"] != "all" and options["fonts"] != ["all"]:
        for ffont in options["fonts"]:
            figify(options["message"], ffont)
        return

    choices = options["size"] if type(options["size"]) == list else [options["size"]]

    # Favorited fonts only.
    if options["favorites"]:
        choices = ["favorites"]

    for category in choices:
        by_category(category, options["message"])
